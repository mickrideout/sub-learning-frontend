...
def auth_user(app, client):
    ...
    with app.app_context():
        ...
        # Login the user by setting up the session
        with client.session_transaction() as sess:
            sess['_user_id'] = str(user.id)
            sess['_fresh'] = True
            sess['_id'] = 'session_id_12345'
...
class TestProgressEndpoints:
    ...
    def test_create_progress_valid(self, client, sample_data, app):
        ...
        response = client.put(
            f"/api/progress/{sample_data['sub_link_id']}",
    ...
    def test_update_existing_progress(self, client, sample_data, app):
        ...
        # Verify in database for update test
        with app.app_context():
            progress = UserProgress.query.filter_by(
                user_id=sample_data['user_id'],
                sub_link_id=sample_data['sub_link_id']
    ...
    def test_get_existing_progress(self, client, sample_data, app):
        ...
        assert 'completion_percentage' in data['progress']
        assert data['progress']['total_alignments'] == sample_data['total_alignments']
    
    def test_update_progress_invalid_data(self, client, sample_data):
    ...
    def test_update_progress_exceeds_total(self, client, sample_data):
        """Test updating progress beyond total alignments."""
        progress_data = {
            'current_alignment_index': sample_data['total_alignments'] + 10,
    ...
    def test_get_recent_progress_with_data(self, client, sample_data, app):
    ...
    def test_get_recent_progress_with_limit(self, client, sample_data, app):
    ...
    def test_progress_unauthorized(self, client, sample_data):
    ...
    def test_completion_percentage_calculation(self, client, sample_data):
        """Test completion percentage calculation accuracy."""
        total_alignments = sample_data['total_alignments']
    ...
    def test_invalid_json_data(self, client, sample_data):
