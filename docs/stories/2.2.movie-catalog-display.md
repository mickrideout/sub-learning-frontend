...
## Tasks / Subtasks
- [ ] Create Movie Catalog Route and Template (AC: 1, 6, 7)
  - [ ] Add `/movies` route in main blueprint for catalog display
  - [ ] Create `app/templates/main/movies.html` template with Bootstrap list layout
  - [ ] Implement movie list display with clean, scannable layout
  - [ ] Add hover states and selection indicators using Bootstrap components

- [ ] Implement Movie Filtering API Endpoint (AC: 3, 4)
  - [ ] Create `GET /api/movies` endpoint in API blueprint
  - [ ] Query sub_links table filtered by user's language pair (native/target)
  - [ ] JOIN with sub_titles to get movie titles with available subtitles
  - [ ] Return JSON response with filtered movies for user's language pair
  - [ ] Add error handling for database connection issues

- [ ] Build Movie Catalog Frontend Component (AC: 1, 2, 6, 7)
  - [ ] Create `app/static/js/modules/movie-discovery.js` component
  - [ ] Implement movie list rendering with clickable titles
  - [ ] Add movie selection handling and navigation
  - [ ] Apply Bootstrap styling for consistent list layout
  - [ ] Add loading states and error handling for API requests

- [ ] Create Movie Selection Navigation (AC: 2, 5)
  - [ ] Add movie selection handling in movie-discovery.js
  - [ ] Implement navigation to learning interface (placeholder redirect)
  - [ ] Store selected movie context in session/state
  - [ ] Add movie selection confirmation and navigation feedback

- [ ] Create Content Service for Movie Discovery (AC: 3, 4)
  - [ ] Add `get_available_movies` method to `app/services/content_service.py`
  - [ ] Implement language pair filtering: `WHERE fromlang=? AND tolang=?`.
  - [ ] Add database transaction handling for movie queries
  - [ ] Include movie metadata and subtitle availability in response

- [ ] Create Movie Catalog Unit and Integration Tests (Testing)
  - [ ] Test movies API endpoint returns filtered movies by language pair
  - [ ] Test content service filters movies correctly for user preferences
  - [ ] Test movie selection navigation and state management
  - [ ] Test error handling for empty movie catalogs and database errors
  - [ ] Test integration with user authentication and language preferences
  - [ ] Test responsive layout and accessibility of movie catalog interface
