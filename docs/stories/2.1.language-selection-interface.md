...
## Tasks / Subtasks
- [ ] Create Language Selection Route and Template (AC: 1, 2, 6)
  - [ ] Add language selection route `/auth/language-selection` in auth blueprint
  - [ ] Create `app/templates/auth/language-selection.html` template with Bootstrap grid layout
  - [ ] Implement dual-panel language selection interface (native/target designation)
  - [ ] Add flag icons display using existing flag images in `app/static/images/flags/`
  - [ ] Style language selection with Bootstrap cards and hover effects

- [ ] Implement Language Data API Endpoint (AC: 1, 6)
  - [ ] Create `/api/languages` endpoint in API blueprint
  - [ ] Query languages table with `SELECT id, name, display_name, code FROM languages`
  - [ ] Return JSON response with all available languages for frontend consumption
  - [ ] Add error handling for database connection issues

- [ ] Build Frontend Language Selection Component (AC: 1, 2, 6, 7)
  - [ ] Create `app/static/js/components/language-selector.js` component
  - [ ] Implement language grid rendering with flag icons and display names
  - [ ] Add native/target language state management using SessionStorage
  - [ ] Implement form validation to ensure both languages selected
  - [ ] Add visual feedback for selected languages and validation errors

- [ ] Create User Language Update Service (AC: 3, 5)
  - [ ] Add `update_user_languages` method to `app/services/auth_service.py`
  - [ ] Update user model: `UPDATE users SET native_language_id=?, target_language_id=? WHERE id=?`
  - [ ] Add database transaction handling for user language updates
  - [ ] Include updated user data in Flask session for persistence

- [ ] Implement Language Selection API Endpoint (AC: 3, 4, 7)
  - [ ] Create `POST /api/user/languages` endpoint in API blueprint
  - [ ] Add request validation using WTForms for native_language_id and target_language_id
  - [ ] Implement same-language validation (native_id != target_id)
  - [ ] Call auth_service.update_user_languages with validated data
  - [ ] Return success response with updated user language preferences

- [ ] Integration with User Registration Flow (AC: 5)
  - [ ] Update OAuth callback handlers to redirect to language selection for new users
  - [ ] Modify email registration success flow to include language selection step
  - [ ] Add session check to skip language selection if already completed
  - [ ] Implement redirect to dashboard after successful language selection

- [ ] Create Language Selection Unit and Integration Tests (Testing)
  - [ ] Test language API endpoint returns all available languages
  - [ ] Test user language update service with valid language IDs
  - [ ] Test same-language validation in API endpoint
  - [ ] Test language selection persistence across sessions
  - [ ] Test integration with OAuth and email registration flows
  - [ ] Test error handling for invalid language IDs and database errors
